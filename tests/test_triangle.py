...
def test_triangle_area_positive(figure_data, type_of_test):
    ...
    assert (
        t.get_area == area
    ), f"Полученная площадь треугольника {t.get_area} не равна ожидаемой - {area}"


@pytest.mark.parametrize(
    "type_of_test", ["integer_triangle_perimeter", "float_triangle_perimeter"]
)
def test_triangle_perimeter_positive(figure_data, type_of_test):
    ...
    ), f"Полученный периметр треугольника {t.get_perimeter} не равен ожидаемому - {perimeter}"


@pytest.mark.parametrize(
    "type_of_test", ["boolean", "negative", "zero", "string", "not_existing_triangle"]
)
